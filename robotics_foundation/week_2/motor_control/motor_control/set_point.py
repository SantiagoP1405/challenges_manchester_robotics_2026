...
class SetPointPublisher(Node):
    ...
    def timer_cb(self):
        ...
        if self.signal_type == 'sine':
            ...
            self.signal_msg.data = self.amplitude * (2/np.pi) * np.arcsin(np.sin(self.omega * elapsed_time))
        # Publish the signal
    ...
    def parameters_callback(self, params):
        for param in params:
            ...
            if param.name == "signal_type":
                ...
                if (param.value not in ['sine', 'square', 'triangle']):
                    self.get_logger().warn("Invalid signal_type! It must be 'sine', 'square', or 'triangle'.")
                    return SetParametersResult(successful=False, reason="signal_type must be 'sine', 'square', or 'triangle'")
                else:
